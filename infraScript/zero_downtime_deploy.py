...
class ServiceManager:
    # 초기화 함수
    def __init__(self, socat_port: int = 8097, sleep_duration: int = 3) -> None:
        self.socat_port: int = socat_port
        self.sleep_duration: int = sleep_duration
        self.services: Dict[str, int] = {
            'here_is_paw_noti_1': 8098,
            'here_is_paw_noti_2': 8099
        }
    ...
    def _find_current_service(self) -> None:
        ...
        if not current_service:
            self.current_name, self.current_port = 'here_is_paw_noti_2', self.services['here_is_paw_noti_2']
        else:
    ...
    def _run_container(self, name: str, port: int) -> None:
        os.system(
            f"docker run -d --name={name} --restart unless-stopped -p {port}:8097 -e TZ=Asia/Seoul -v /dockerProjects/here_is_paw_msa/notification/volumes/gen:/gen --pull always ghcr.io/here-is-paw/here_is_paw_noti")
    ##
